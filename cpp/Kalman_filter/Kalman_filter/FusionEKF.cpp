...
FusionEKF::FusionEKF() {
  ...
  R_DVL_ = MatrixXd(2, 2);
  R_SONAR_ = MatrixXd(3, 3);
  H_DVL_ = MatrixXd(2, 4);
  H_jacobian = MatrixXd(3, 4);

  //measurement covariance matrix - DVL - not sure
  R_DVL_ << 0.0225, 0,
              0, 0.0225;

  //measurement covariance matrix - SONAR - not sure
  R_SONAR_ << 0.09, 0, 0,
              0, 0.0009, 0,
              0, 0, 0.09;
   ...
   ****************************************************************************/
	  if (!is_initialized_) {
		...
		//    cout << "EKF: " << endl;
			...
			if (measurement_pack.sensor_type_ == MeasurementPackage::SONAR) {
			  /**
			...
			else if (measurement_pack.sensor_type_ == MeasurementPackage::DVL) {
			  /**
			  Initialize state.
			  */
	  ...
	  // print the output
	  cout << "measurement update KF" << endl;
	  cout << "x_ = " << ekf_.x_ << endl;
	  cout << "P_ = " << ekf_.P_ << endl;
	  cout << endl << endl;
